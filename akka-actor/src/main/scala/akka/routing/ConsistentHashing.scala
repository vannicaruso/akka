...
object ConsistentHashingRouter {
   ...
   */
  @deprecated("Use ConsistentHashingNozzle", "2.3")
...
/**
 * Uses consistent hashing to select a connection based on the
 * sent message.
 *
...
/**
 * A router pool that uses consistent hashing to select a connection based on the
 ...
 */
@SerialVersionUID(1L)
final case class ConsistentHashingNozzle(
   ...
   * router management messages
   */
  def withDispatcher(dispatcherId: String): ConsistentHashingNozzle = copy(routerDispatcher = dispatcherId)

  /**
   * Setting the number of virtual nodes per node, used in [[akka.routing.ConsistentHash]]
   */
  def withVirtualNodesFactor(vnodes: Int): ConsistentHashingNozzle = copy(virtualNodesFactor = vnodes)

  /**
   * Java API: Setting the mapping from message to the data to use for the consistent hash key.
   */
  def withHashMapper(mapper: ConsistentHashingRouter.ConsistentHashMapper): ConsistentHashingNozzle =
    copy(hashMapping = ConsistentHashingRouter.hashMappingAdapter(mapper))
  ...
  override def withFallback(other: RouterConfig): RouterConfig = other match {
    ...
    case otherRouter: ConsistentHashingNozzle ⇒ copy(hashMapping = otherRouter.hashMapping)
    case _                                    ⇒ throw new IllegalArgumentException("Expected ConsistentHashingNozzle, got [%s]".format(other))
  }
 ...
 */
@SerialVersionUID(1L)
@deprecated("Use ConsistentHashingPool or ConsistentHashingNozzle", "2.3")
