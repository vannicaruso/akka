...
final case class SmallestMailboxPool(
   ...
   */
  def withResizer(resizer: Resizer): SmallestMailboxPool = copy(resizer2 = Some(resizer))
...
/**
 * A router nozzle that tries to send to the non-suspended routee with fewest messages in mailbox.
 * The selection is done in this order:
 * <ul>
 * <li>pick any idle routee (not processing message) with empty mailbox</li>
 * <li>pick any routee with empty mailbox</li>
 ...
 */
@SerialVersionUID(1L)
final case class SmallestMailboxNozzle(
   ...
   * router management messages
   */
  def withDispatcher(dispatcherId: String): SmallestMailboxNozzle = copy(routerDispatcher = dispatcherId)
