...
import akka.routing2.NoRoutee
import akka.routing2.NoRouter
...
final case class AdaptiveLoadBalancingPool(
  ...
  override def resizer2: Option[Resizer] = None
  ...
  override def withFallback(other: RouterConfig): RouterConfig = {
    if (other == NoRouter) this // NoRouter is the default, hence “neutral”
      ...
      this.withSupervisorStrategy(other.supervisorStrategy)
    else this
  }
}

/**
 * A router nozzle that performs load balancing of messages to cluster nodes based on
 * cluster metric data.
 *
 * It uses random selection of routees based on probabilities derived from
 * the remaining capacity of corresponding node.
 ...
 */
@SerialVersionUID(1L)
final case class AdaptiveLoadBalancingNozzle(
  metricsSelector: MetricsSelector = MixMetricsSelector,
   ...
   * router management messages
   */
  def withDispatcher(dispatcherId: String): AdaptiveLoadBalancingNozzle = copy(routerDispatcher = dispatcherId)
